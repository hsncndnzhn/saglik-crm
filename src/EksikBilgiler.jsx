...
export default function EksikBilgiler() {
  ...
  async function saveEdit(r) {
    setSaving(true)
    await supabase.from('patients').update({ passport_no: form.passport_no, address: form.address }).eq('id', r.id)
    setSaving(false)
    setEditing(null)
  ...
  return (
    <div>
      ...
      <div style={{ position: 'relative', marginBottom: '1rem' }}>
        <input
          type="text"
          placeholder="Ad, soyad veya hasta kodu ile ara..."
          value={search}
      ...
      {filtered.length === 0 && (
        <div style={{ background: 'white', border: '1px solid #eee', borderRadius: '10px', padding: '3rem', textAlign: 'center', color: '#888', fontSize: '13px' }}>
          {rows.length === 0 ? 'Eksik bilgisi olan hasta yok.' : 'Sonuç bulunamadı.'}
        </div>
      )}

      {filtered.map(r => (
        <div key={r.id + '_' + r.sale_id} style={{ background: 'white', border: '1px solid #eee', borderRadius: '10px', padding: '14px', marginBottom: '8px' }}>
          <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: editing === r.id ? '12px' : '0' }}>
            <div>
              ...
              <div style={{ fontSize: '11px', color: '#888', marginTop: '2px' }}>{r.salesperson_name}</div>
          ...
          {editing === r.id && (
            <div>
              <div style={{ marginBottom: '8px' }}>
                ...
                <input type="text" value={form.passport_no} onChange={e => setForm(f => ({ ...f, passport_no: e.target.value }))}
              ...
              <div style={{ marginBottom: '10px' }}>
                ...
                <textarea value={form.address} onChange={e => setForm(f => ({ ...f, address: e.target.value }))}
                  placeholder="Sokak, mahalle, şehir, ülke"
                  rows={2}
                  style={{ width: '100%', padding: '8px 10px', fontSize: '13px', border: '1px solid #ddd', borderRadius: '6px', resize: 'none' }} />
              </div>
              <div style={{ display: 'flex', gap: '8px' }}>
                <button onClick={() => setEditing(null)}
                  style={{ flex: 1, padding: '8px', fontSize: '12px', border: '1px solid #ddd', borderRadius: '6px', background: 'white', cursor: 'pointer', color: '#555' }}>
                  İptal
                </button>
